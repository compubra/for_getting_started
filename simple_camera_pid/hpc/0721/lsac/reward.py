...
class QuadraticReward:
    ...
    e is the vision steering_error only (no pose information). u is the
    residual action — since 2026-07-21 a 2-D [delta_v, delta_omega] (was a
    scalar delta_omega before that day's Diff_Drive_Kinematics refactor);
    ||u||^2 is the sum of squares across whatever u's length is, matching the
    MATLAB side's USq_Demux + Sum reduction after the action space grew.
    ...
    def step(self, e_steer: float, e_lat: float, e_head: float,
        ...
        p = self.params
        return (-(p.q * e_steer ** 2 + p.r * _action_energy(u))
