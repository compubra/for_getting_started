"""Per-episode domain randomization — translation of sac_dr_defaults.m +
sac_domain_randomization.m.
...
class DRConfig:
    ...
    # Per-episode freshly generated track (ellipse/capsule loop via
    # track_gen.gen_simple_track_scene). Takes priority over Map when
    # enabled — mirrors MATLAB's GenTrack group. Off by default: it's
    ...
    # single-map overfitting this session's generalization check found.
    gen_track_enable: bool = False
    gen_track_shape: str = "random"        # ellipse | capsule | random
    gen_track_difficulty: float = 0.4      # 0..1, higher = smaller/tighter loop
...
def sample_episode(rng: np.random.Generator, cfg: DRConfig,
    ...
    if cfg.gen_track_enable:
        from .track_gen import gen_simple_track_scene
        ...
        map_key = info.scene_file  # resolve_scene() also accepts a raw path
        is_generated = True
    ...
    if cfg.control_enable:
        ctrl.base_linear_speed = _pct(rng, base_ctrl.base_linear_speed, cfg.base_speed_pct)
