...
(model visual_line_follower_sac_residual.slx), updated 2026-07-23 to track
that model's current state (supersedes hpc/0721/train_sac_residual.py):

Reward (Reward_Calculation subsystem — 2026-07-23 gained the Q_Lateral/
Q_Heading terms, wiring verified via model_read on the .slx after the edit):
  reward(t) = -(Q*e^2 + Q_lat*e_lat^2 + Q_head*e_head^2 + R*||u||^2)
              - P_Lost*(1-found)
e is the vision steering_error; e_lat/e_head are lateral_error/heading_error
from the same vision subsystem — no pose information enters the reward.
...
Domain randomization (rl_dr_defaults.m; 2026-07-23 reading of the current
MATLAB source): GenTrack.Enable is actually `true` (its own inline comment
claiming "default off" is stale — the cached 2026-07-21 500-episode live-
script run generated a fresh track every episode, confirming Enable=true is
what that run actually trained against), with Generator="random" picking
among three algorithms each episode:
  simple    — gen_simple_track_scene:    ellipse / capsule
  complex   — gen_complex_track_scene:   polyline + independent corner-arc
                                          fillets, sharp-cornered look
  wandering — gen_wandering_track_scene: smooth spline-interpolated organic
                                          winding loop
--gen-track is therefore ON by default here too (was opt-in in hpc/0721);
pass --no-gen-track to train on a fixed --map-key instead.
...
Training loop mapping:
  ...
  UseDomainRand=true, DomainRand.Map.Enable=false, GenTrack.Enable=true
                                            → DRConfig(map_enable=False,
                                              gen_track_enable=True)
...
⚠️ 2026-07-21 same-day training-run finding (still applies): with
BaseSpeedMax at the full Burger-rated 7.333 (0.22 m/s), a 500x200 run on a
fixed map never converged (flat ~-260 average reward for all 500 episodes) —
at high sampled base speeds there's too little steering headroom left under
MaxWheelSpeed before saturating, so a large fraction of episodes are
physically uncorrectable regardless of policy quality. Consider passing a
lower --base-speed-max (e.g. ~5.6, leaving ~60-70% of MaxWheelSpeed for the
forward component) until this is re-validated under the new default
gen-track-on regime.

Train on a fixed map instead of the (now-default) procedurally generated
tracks:
  python train_sac_residual.py --no-gen-track --map-key simple
"""
...
def build_env(args) -> Monitor:
    ...
    if args.domain_rand:
        ...
        # GenTrack.Enable=true (see this file's module docstring — the
        # "default off" comment on the MATLAB side is stale) — pose/dynamics/
        # PID/perception randomized, tracks freshly generated every episode
        # via one of the three track_gen algorithms (--gen-track-generator).
        ...
        # too little steering headroom to converge on a fixed map.
        dr = DRConfig(map_enable=args.rand_map, gen_track_enable=args.gen_track,
                     gen_track_generator=args.gen_track_generator,
    ...
    reward = QuadraticReward(QuadraticRewardParams(
        q=args.q, q_lateral=args.q_lateral, q_heading=args.q_heading,
        r=args.r_u, p_lost=args.p_lost))
...
def main() -> None:
    ...
    p.add_argument("--map-key", default="simple",
                   help="ignored unless --no-gen-track is set")
    ...
    p.add_argument("--base-speed-max", type=float, default=0.22 / 0.03,
                   help="DomainRand.Control.BaseSpeedMax; default is the "
                        ...
                        "converge in the 2026-07-21 fixed-map run")
    p.add_argument("--no-gen-track", dest="gen_track", action="store_false",
                   help="train on the fixed --map-key instead of a fresh "
                        "procedurally generated track every episode. "
                        "GenTrack is ON by default (needs --domain-rand, "
                        "the default), matching rl_dr_defaults.m's actual "
                        "GenTrack.Enable=true -- see module docstring")
    p.add_argument("--gen-track-generator", default="random",
                   choices=["simple", "complex", "wandering", "random"],
                   help="which track_gen algorithm generates the episode "
                        "track; 'random' (default) picks one of the three "
                        "each episode, matching rl_dr_defaults.m's "
                        "dr.GenTrack.Generator")
    p.add_argument("--gen-track-shape", default="random",
                   choices=["ellipse", "capsule", "random"],
                   help="only consulted when --gen-track-generator resolves "
                        "to 'simple'")
    p.add_argument("--gen-track-difficulty", type=float, default=0.4,
                   help="0..1, higher = tighter/more irregular generated "
                        "loop; shared semantics across all three generators")
    ...
    p.add_argument("--rand-map", action="store_true",
                   help="DomainRand.Map.Enable=true -- randomize --map-key "
                        ...
                        "map. Ignored unless --no-gen-track is also set "
                        "(gen-track takes priority, same as MATLAB).")
    p.add_argument("--seed", type=int, default=None)
    # ── reward weights (sac_training_config.m §3) ──
    p.add_argument("--q", type=float, default=1.0, help="SAC_Q")
    p.add_argument("--q-lateral", type=float, default=0.5, help="SAC_Q_Lateral")
    p.add_argument("--q-heading", type=float, default=0.5, help="SAC_Q_Heading")
    ...
    env = build_env(args)
    if args.gen_track:
        print(f"Training map: procedurally generated (generator="
              f"{args.gen_track_generator}, shape={args.gen_track_shape}, "
    ...
    print(f"Reward weights: Q={args.q} Q_lat={args.q_lateral} "
          f"Q_head={args.q_heading} R={args.r_u} P_lost={args.p_lost} "
