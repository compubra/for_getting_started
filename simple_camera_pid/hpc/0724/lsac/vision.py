...
Input is a (480, 640, 3) RGB image with values 0..255 (float or uint8), row 0
...
(640x480 confirmed 2026-08-03 against real hardware -- the real TurtleBot3's
camera driver was reconfigured to this resolution to match MuJoCo/Gazebo, so
this file's original 640x480 was correct all along; a same-day 800x600 sync
attempt was reverted once the real camera's own resolution changed back.
CAM_PITCH_DEG=15.0 was separately confirmed correct against the real camera
too -- see that constant's comment.)
...
IMAGE_HEIGHT = 480  # confirmed 2026-08-03 against the real camera
IMAGE_WIDTH = 640   # confirmed 2026-08-03 against the real camera
MAX_PATH_POINTS = 30
...
CAM_PITCH_DEG = 15.0  # confirmed against the real camera 2026-08-03, unchanged
...
class LocalPathGenerator:
    ...
    # model-workspace-tunable). Pixel units -- describe real-world feature
    # sizes in pixels, not angles, so they scale linearly with image_width;
    # at 640 wide (confirmed 2026-08-03 against the real camera) that ratio
    # is exactly 1.0, so these are their original values, unchanged.
