...
%
% ⚠️ 归档说明（2026-07-22）：这是方案 A 时代（分带扫描，未做最大连通块
% 筛选、不含霍夫/滑动窗口/多项式拟合）的原始调试帧实现，函数名当时与现在
% 相同（originbot_line_follower_debug_frame）。本文件内容是从本次会话早
% 前一次 Read 工具调用的输出逐字重建的——runtime/ 里的当时版本已在本次
% 归档动作之前、由本会话之外的另一次修改覆盖为方案 B 风格（含霍夫+滑窗+
% 拟合，但仍是 MuJoCo 单平台/15° 硬编码），本文件是这条演化链最早一环唯一
% 留存的副本。若要恢复，需把文件名改回 originbot_line_follower_debug_
% frame.m 放回 runtime/，且注意它是方案 A 专用（分带扫描 ROI/掩码选取
% 逻辑与方案 A 算法一致，与现行方案 B 控制器不匹配，画面会与实际转向脱节）。
%
...
% MuJoCo framebuffer 为 bottom-row-first，raw 帧上下颠倒（左右正常），翻正一次，
% 与控制器 originbot_local_path_generator 保持一致（否则 Camera Monitor 显示颠倒）。
rgb = flip(rgb, 1);
...
% 单向检测（不再做“正向 vs 180°翻转取白像素多者”的择优：raw 是纯 flipud，
% 上面已显式翻正；旧择优在 V 弯会误切换朝向，导致调试画面 180° 来回倒转）。
[mask, pixelCount, centroidX] = detectMask( ...
