#!/usr/bin/env python
"""Smoke check for the residual-model spec (7-D obs, SteeringSign=-1 + upright
camera, ROIFraction=0.5) before the full SAC/PPO runs.
...
that both SB3 agents train a few episodes end-to-end on the 7-D observation.
...
def check_pid_tracks() -> None:
    ...
    assert obs.shape == (7,), f"expected 7-D obs, got {obs.shape}"
    found = 0
    n = 400  # 20 s PID-only
    for k in range(n):
