...
class ImageSubscriber(Node):
  ...
  def __init__(self):
    ...
    self.y = -1
    self.uuid = -1
    self.image = np.zeros((960, 600, 3))
    self.initialize = False

    # 检查用户存放标定数据的image_dataset文件夹是否存在，如果不存在则创建
    if not os.path.exists('～/桌面/obot-sim/src/line_follower_model/image_dataset'):
      os.makedirs('～/桌面/obot-sim/src/line_follower_model/image_dataset')

    # 设置opecv展示窗属性
    cv.namedWindow("capture image", cv.WINDOW_NORMAL)
    cv.resizeWindow("capture image", 960, 600)
    self.subscription
  ...
  def listener_callback(self, msg):
    ...
    if keyValue == 13:
      ...
      cropImage = captureImage.copy()#相关参数要根据场地情况作调整
      ...
      if self.x != -1 and self.y != -1:
        ...
        # 保存上一次标定的结果
        cv.imwrite('～/桌面/obot-sim/src/line_follower_model/image_dataset/' + self.uuid + '.jpg', self.image)
        # 载入新的图像
        self.image = cropImage.copy()
        self.x = -1
        self.y = -1
